...
from utils import check_version, make_anchors, dist2bbox
from models.layers.common_layer import autopad, Conv
...
class Classify(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.flat(self.conv(z))  # flatten to x(b,c2)
        # print(out)
        return out if self.training else (out.sigmoid() if self.use_sigmoid else F.softmax(out, dim=1), out)
...
class Classify2(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g)  # to x(b,c2,h,w)
        self.aap = nn.AdaptiveAvgPool2d(1)  # to x(b,c1,1,1)
        self.flat = nn.Flatten()
        if c2==1:
            self.use_sigmoid = True
    ...
    def forward(self, x):
        ...
        out = self.flat(self.aap(self.conv(x)))  # flatten to x(b,c2)
        # print(out)
        return out if self.training else (out.sigmoid() if self.use_sigmoid else F.softmax(out, dim=1), out)
        # return out if self.training else (out.sigmoid(), out)
...
class IDetect(nn.Module):
    ...
    def __init__(self, ch=(), nc=80, anchors=()):  # detection layer
...
class YOLOv8Classify(nn.Module):
...
class SqueezenetClassify(nn.Module):
