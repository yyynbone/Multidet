...
from utils import ( check_dataset, check_img_size, check_suffix, colorstr, time_sync, init_seeds, cal_flops, intersect_dicts,
                    ...
                    output_to_target, xywh2xyxy, xyxy2xywh, non_max_suppression_with_iof, div_area_idx, process_batch, area_filter, calcu_per_class, ConfusionMatrix,
                    select_class_tuple, save_one_txt, save_one_json, select_score_from_json, plot_results, plot_images,  visual_match_pred, print_log, visual_return,classify_match, save_object )
...
def train(hyp, opt, logger, device, Local_rank=-1, Node=-1, World_size=1):
    ...
    if pretrained and len(csd) == len(model.state_dict()):
        ...
        if epochs < start_epoch:
            print_log(f"{weights} has been trained for {ckpt['epoch']} epochs. Fine-tuning for {epochs} more epochs.", logger)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if not opt.val_first:
            ...
            if hasattr(compute_loss, 'pos_weight'):
                ...
                print_log(f'now epoch_pos_w is {epoch_pos_w}, ClassifyLoss label_pos_weight is {compute_loss.label_pos_weight}, ClassifyLoss pos_weight is {compute_loss.pos_weight}', logger)
        ...
        for li, conf_t in enumerate(last_conf):
            ...
            if verbose and nc > 1 and len(stats):
                for i, c in enumerate(ap_class):
                    if c in names:
                        print_log(pf % (names[c], nt[i, 0], pred_truth[i, 0], lev_np[i, 0], p[i, 0], r[i, 0], ap50[i, 0], ap[i, 0]), logger)
                        if isinstance(div_area, (list, tuple)):
                            for div_i, div_ang in enumerate(div_area):
                                print_log(pf % (f'{names[c]}_under_{div_ang}', nt[i, div_i+1], pred_truth[i, div_i+1], lev_np[i, div_i+1], p[i, div_i+1], r[i, div_i+1], ap50[i, div_i+1], ap[i, div_i+1]), logger)
                            print_log(pf % (
