...
from loss import *
from loss.loss_grad_hook import Loss_Auto_Weight_Hook
from models  import (attempt_load, Model)
from dataloader import check_anchors,  train_val_split, check_train_batch_size, BalancedDataParallel,\
    LoadImagesAndLabels, get_dataset, SuffleLoader, SuffleDist_Sample
...
def train(opt, logger, device, train_data, train_pre, val_data, val_pre,  local_rank=-1, node=-1, world_size=1):
    ...
    if pretrained:
        ...
        if isinstance(ckpt, dict):
            if ckpt.get('ema', None) is not None:
                ...
                csd = intersect_dicts(csd, model.state_dict(), exclude=exclude, logger=logger)  # intersect
            elif ckpt.get('model', None) is not None:
                csd = ckpt['model'].float().state_dict()
                csd = intersect_dicts(csd, model.state_dict(), exclude=exclude, logger=logger)  # intersect
            else:
                csd = ckpt['state_dict']
                csd = intersect_dicts(csd, model.state_dict(), exclude=exclude, key_match=False, logger=logger)  # intersect
        else:
            csd = intersect_dicts(ckpt, model.state_dict(), exclude=exclude, key_match=False, logger=logger)  # intersect
    ...
    fs = cal_flops(model, imgsz)
    print_log(f'now with size of {imgsz}, {fs:.2f} GFLOPS\n', logger)
    ...
    hyp['cls_weight'] = class_weights.tolist()
    opt.hyp = hyp
    if opt.rect:
    ...
    print_log(f"Epoch train image shape is {train_dataset[0][0].shape}, should input as (ch, y, x)"
    ...
    for k, v in model.named_parameters():
        if v.requires_grad is False:
            print_log(f'{k}, required is False', logger)
        # v.requires_grad = True  # train all layers  # 如果有某个层在forward 中不参与计算，如yolov8 df1，此时如果requires_grad设为True, ddp 需要find_unused_parameters: True， 否则报错。
    ...
    if cuda and node == -1 and torch.cuda.device_count() > 1:
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  modify
        # model = BalancedDataParallel(batch_size-2, model,dim =0).to(device)
    ...
    model.class_weights = torch.from_numpy(class_weights) * model.nc  # attach class weights
    ...
    nw = max(round(hyp['warmup_epochs'] * nb), 100*256/batch_size)  # number of warmup iterations, max(3 epochs, 1k iterations)
    ...
    loss_key = ['box', 'cls', 'dfl'] if opt.loss == 'LossV8' else ['box', 'obj', 'cls']
    if loss_num == 4:
        loss_key.append('seg')

    Loss_weight = Loss_Auto_Weight_Hook(model, compute_loss, detect_epoch=0, grad_detect=False, adj_epoch=50, start_epoch=start_epoch,loss_name=loss_key, logger=logger)
    ...
    if node in [-1, 0] and local_rank in [-1, 0]:
        ...
        if loss_num != 1:
            par_msg = '%10s' * (4 + loss_num) % ('Epoch', 'gpu_mem', *loss_key, 'labels', 'img_size')
            cbkey = []
            for key in loss_key:
                cbkey.append(f'train/{key}_loss')
            cbkey.extend(['metrics/precision', 'metrics/recall', 'metrics/mAP_0.5',
                          'metrics/mAP_0.5:0.95'])
            for key in loss_key:
                cbkey.append(f'val/{key}_loss')
            cbkey.extend(['x/lr0', 'x/lr1', 'x/lr2'])
            callbacks.keys = cbkey
        # elif loss_num == 4:
    ...
    for _ in range(10000):
        ...
        if not opt.val_first:
            ...
            if  epoch%getattr(opt, 'shuffle_epoch', 1000)==0:
                print_log(f'now we shuffle index in {epoch}', logger)
                train_loader.shuffle_index()
                if node in [-1, 0] and local_rank in [-1, 0]:
                    val_loader.shuffle_index()
                print_log(f'now train shuffle and now dataset length is {len(train_loader)}', logger)
                print_log(f'now val shuffle and now dataset length is {len(val_loader)}', logger)
            ...
            Loss_weight.before_train_epoch(epoch)
            ...
            for i, (imgs, targets, paths) in pbar:  # batch -------------------------------------------
                ...
                Loss_weight.after_train_iter(i, loss_items)
                ...
                if node in [-1, 0] and local_rank in [-1, 0]:
                    ...
                    if loss_num == 1:
                        descrip_item =  (*loss_items, targets[0].shape[0], targets[0].sum(),
                            targets[1].shape[0])
                    else:
                        descrip_item = (*loss_items, targets[1].shape[0])
                    pbar.set_description('in each iter:'+ '%10s'*2%(f'{epoch}/{epochs - 1}', mem) + '%10.4g' * len(descrip_item)%descrip_item +f'{list(imgs.shape)}' )
            ...
            scheduler.step()
            if node in [-1, 0] and local_rank in [-1, 0]:
                descrip_item = (*mloss, *(descrip_item[loss_num:]))
                print_log('in all epoch:'+ '%10s'*2%(f'{epoch}/{epochs - 1}', mem) + '%10.4g' * len(descrip_item)%descrip_item+f'{list(imgs.shape)}', logger)
                if opt.loss_supervision:
                    lr_coeft = out_saddle(epoch=epoch, loss=loss.item())
                    if lr_coeft > 1:
                        print_log(f'now in epoch {epoch}, lr coefficient is:{lr_coeft}', logger)

        Loss_weight.after_train_epoch(epoch)
    ...
    # end training ------------------------------------------------
    if node in [-1, 0] and local_rank in [-1, 0]:
        print_log(f'\n{epoch +1 - start_epoch} epochs completed in {(time.time() - t0) / 3600:.3f} hours.', logger)
...
def val(data,
    ...
    for batch_i, (im, targets, paths) in enumerate(pbar):
        ...
        for path in paths:
            img_name_f.write(f'{Path(path).stem}\n')
        t1 = time_sync(device)
