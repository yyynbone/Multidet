...
from dataloader.check_load import IMG_FORMATS, VID_FORMATS, img2label_paths, img2seglabel_paths, attach_labels, load_mosaic, load_image

from dataloader.augmentations import Albumentations, augment_hsv, letterbox, mixup, random_perspective, Clahe
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path,  img_size=640, batch_size=16, logger=LOGGER, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.int) * stride # hs,ws = img_size,  if h>w, (hs,w*ws/h) else (hs*h/w,ws)
        '''
