...
from utils import  check_version, colorstr, bbox_ioa, resample_segments, segment2box, xywh2xyxy, xyxy2xywh, print_log
...
class Albumentations:
    def __init__(self, logger):
        ...
        try:
            ...
            print_log(colorstr('albumentations: ') + ', '.join(f'{x}' for x in self.transform.transforms if x.p), logger)
        except ImportError:  # package not installed, skip
            pass
        except Exception as e:
            print_log(colorstr('albumentations: ') + f'{e}', logger)
    def __call__(self, result, p=1.0):
        if self.transform and random.random() < p:
            new = self.transform(image=result['img'], bboxes=result['labels'][:, 1:], class_labels=result['labels'][:, 0])  # transformed
            result['img'], result['labels'] = new['image'], np.array([[c, *b] for c, b in zip(new['class_labels'], new['bboxes'])])
...
def adapt_pad(result, pad_color=(114, 114, 114),seg_pad_color=(0, 0, 0), auto=True, scaleFill=False, scaleup=True):
    ...
    if labels is not None:
        if labels.size:  # normalized xywh to pixel xyxy format
            labels[:, 1:] = xywh2xyxy(labels[:, 1:], ratio[0] * result['resized_shape'][1], ratio[1] * result['resized_shape'][0], padw=dw, padh=dh)
            result['labels'] = labels
            # result['instance_segment'] =
...
def resize(result, reshape, augment=False):
    result['img_size'] = reshape
    h, w = result['img'].shape[:2]
    r = min(reshape[0] / h, reshape[1] / w)  # ratio
...
def gray(result):
    result['img'] = cv2.cvtColor(result['img'], cv2.COLOR_BGR2GRAY)[...,None]   #(h, w, 1)
    if result['segment'] is not None:
