...
from utils import  segments2boxes, print_log, check_version, colorstr, xyxy2xywh, xywh2xyxy, xyn2xy
...
def rect_shape(results, pad, stride, batch_size):
    ...
    batch_shapes = np.ceil(np.array(shapes) / stride + pad).astype(np.int8) * stride
...
def album(transform, result, p=1.0):
